    ...
    for v in iter_vision(map_data, filter_target) do
        ...
        if dist_sq < observe_range_sq then
            ...
            if data.id == g_selected_target_id and data.type == g_selected_target_type then
                ...
                if v:get_is_air_target() then
                    is_show_target_distance = true
                end
            end
    ...
    local function render_target_vehicle_info(pos, data, col)
        if data.is_laser_target then
            ...
            if data.team == vehicle_team then
                ...
                ux_render_hover_info(data, pos, cursor_y, col)
                ...
                if data.vehicle:get_is_observation_weapon_revealed() then
                    update_ui_text(pos:x() + 9, cursor_y + 4, get_vehicle_capability(data.vehicle), 200, 0, col, 0)
            ...
            if is_show_target_distance and data.is_observed then
                ...
                -- ux - move target distance down half a line else obsucred by cam orientation pointer
...
g_ux_screen_team = nil
g_ux_hidden_attachments = {
...
function ux_list_contains(list, x)
    for _, list_value in pairs(list) do
		if list_value == x then return true end
	end
	return false
end

function ux_value_in(value, list)
...
function ux_render_hover_info(data, pos, cursor_y, col)
    local st, err = pcall(function()
        if g_ux_screen_team == nil then
            g_ux_screen_team = update_get_screen_team_id()
        end

        if data.vehicle ~= nil then

            local id_x = pos:x() + 8
            local id_y = cursor_y - 12
            local idstr = string.format("ID %d", data.vehicle:get_id())

            if data.vehicle:get_definition_index() ~= e_game_object_type.chassis_carrier then
            ...
            local attachment_y = cursor_y + 24
                if data.ux_friendly then
                    ...
                    -- render basic attachment details
                    local a_count = data.vehicle:get_attachment_count()
                    local a_seen = {}
                    for i = 0, a_count - 1 do
                        local attachment = data.vehicle:get_attachment(i)
                        if attachment ~= nil then
                            local a_def = attachment:get_definition_index()
                            if a_def > 0 and a_def < e_game_object_type.count then
                                if not ux_list_contains(a_seen, a_def) then
                                    if not ux_list_contains(g_ux_hidden_attachments, a_def) then
                                        local a_name = get_attachment_display_name(data.vehicle, attachment)
                                        update_ui_text(pos:x() + 8, attachment_y - 10, a_name, 200, 0, col, 0)
                                        attachment_y = attachment_y + 9
                                    end
                                end
                            end
                            table.insert(a_seen, a_def)
                        end
                    end
                end
            end
            update_ui_text(id_x, id_y, idstr, 200, 0, col, 0)
        end
    end)
    if not st then
        ux_dbg(err)
...
function ux_update_per_unit(data, v)
    if v:get_is_docked() then
        -- hide docked units from the hud
        data.is_observed = false
    end
    data.ux_friendly = g_ux_screen_team == v:get_team_id()
    data.ux_is_fully_revealed = false
    data.ux_is_type_revealed = false

    if v:get_is_observation_type_revealed() then
        data.ux_is_type_revealed = true
    end
    if v:get_is_observation_fully_revealed() then
        data.ux_is_fully_revealed = true
    end
end



g_ux_func_override_get_vehicle_capability = get_vehicle_capability


function get_vehicle_capability(vehicle)
    -- return more info for guns on turrets or ground units
    ...
    local result = join_strings(capabilities, "/")
    if result == "" then
        return g_ux_func_override_get_vehicle_capability(vehicle)
    end
