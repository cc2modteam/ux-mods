        ...
        if selected_waypoint:get() then
            ...
            local window = ui:begin_window(update_get_loc(e_loc.upp_waypoint), 30, 30, screen_w - 60, screen_h - 60, atlas_icons.column_distance, true, 2)
                ...
                if get_is_vehicle_air(vehicle_definition_index) then
                    ...
                    -- waypoInt altitude selectors
                    waypoint_altitude, is_modified = ui:selector(update_get_loc(e_loc.upp_altitude), waypoint_altitude, 50, 2000, 50)
                    local alt_adj = ui:button_group({"-500", "-100", "+100", "+500", "+1000"}, true)
                    if alt_adj > -1 and alt_adj < 5 then
                        local modify = 0
                        if alt_adj == 0 then
                            modify = -500
                        elseif alt_adj == 1 then
                            modify = -100
                        elseif alt_adj == 2 then
                            modify = 100
                        elseif alt_adj == 3 then
                            modify = 500
                        elseif alt_adj == 4 then
                            modify = 1000
                        end
                        waypoint_altitude = waypoint_altitude + modify
                        is_modified = true
                    end
                    if waypoint_altitude < 0 then
                        waypoint_altitude = 0
                    elseif waypoint_altitude > 2030 then
                        waypoint_altitude = 2030
                    end
